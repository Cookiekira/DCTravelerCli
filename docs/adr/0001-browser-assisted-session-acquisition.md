...
Accepted: DC Traveler CLI uses a real Chromium-family browser login flow to acquire FF14 China session cookies, then performs official API calls through typed .NET HTTP clients after the session has been extracted. This keeps WeGame/QQ authentication, redirects, and human verification inside the browser while avoiding brittle DOM click automation and page-injected JavaScript fetches in the business workflow.

## Consequences

- CDP is an infrastructure detail for launching or attaching to a supported Chromium-family browser, observing login success, and extracting cookies.
- Browser discovery prefers Chrome-compatible installs, then falls back to other common Chromium-based browsers. Users can still provide an explicit browser path when automatic discovery misses their environment.
- After cookie extraction, the CLI saves the official session locally and future travel runs probe that saved session before opening a browser.
