...
The existing release workflow produces Windows x64 zip assets and checksums on GitHub Releases. Winget portable command aliases expect the executable to remain runnable from the alias location, so the winget manifest uses the Windows x64 AOT asset. The ordinary Windows x64 release should still be single-file for manual GitHub Releases downloads, but the winget package should stay on the AOT asset unless a later release deliberately changes that distribution decision.

## Consequences

- Stable GitHub Releases remain the source of binary release assets.
- The package-manager manifest targets the stable Windows x64 AOT zip asset.
